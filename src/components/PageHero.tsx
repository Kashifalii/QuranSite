...
export function PageHero({ title, subtitle, image = "/images/contact-quran.jpg" }: PageHeroProps) {
  return (
    <section className="relative grid min-h-[420px] place-items-center overflow-hidden bg-black text-center text-white">
      ...
      <div className="container-page relative z-10 pt-28">
        ...
        <h1 className="font-display text-4xl font-bold uppercase leading-tight sm:text-5xl">{title}</h1>
        {subtitle ? <p className="mx-auto mt-4 max-w-2xl text-white/82">{subtitle}</p> : null}
      </div>
    </section>
  );
}
